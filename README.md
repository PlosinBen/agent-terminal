...
client/ (Vite + React)          shared/
  ...
  components/
    ...
    Terminal, StatusLine,           copilot/ (OpenAI SDK)
    SettingsPanel                   mock/    (E2E testing)
                                    registry.ts
  stores/                        handlers/
...
- **多 Agent Provider** — 可擴展的 Provider 架構（Claude、Gemini、GitHub Copilot），專案建立時選擇 Provider
...
| Copilot API | OpenAI SDK (api.githubcopilot.com) |
| PTY | node-pty |
...
│   ├── copilot/
│   │   ├── index.ts            Copilot provider definition
│   │   ├── backend.ts          OpenAI SDK agent loop
│   │   ├── auth.ts             GitHub token → Copilot token exchange
